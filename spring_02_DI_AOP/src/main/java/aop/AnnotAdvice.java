...
public class AnnotAdvice {
	...
	@Pointcut("execution(* spring.biz.*.service.*.*(..))")
	...
	public void ex(JoinPoint jp, Exception e) { //jp, e 순서 바뀌면 에러남. jp는 반드시 첫 번째 파라미터
		LOGGER.error(jp.getSignature()+" => "+e.getMessage());
	}
}
